import spanStyles from "./TableStatusButton";
import { ChannelProps } from "../../types";
import editicon from '../../assets/editicon.svg';
import useChannelStore from "../../store/channelStore";
import { useState, useEffect } from "react";

const TableRowStyle: React.FC<ChannelProps> = ({ id, channel_number, product_id, package_name, channel_name }) => {
    const { editChannel } = useChannelStore();
    const [isEditing, setIsEditing] = useState(false);
    const [editData, setEditData] = useState<ChannelProps>({
        id,
        channel_number,
        product_id,
        package_name,
    ...
    return (
        <tr className="bg-white">
            ...
            <td className="p-3 text-sm text-gray-700 whitespace-nowrap flex items-center space-x-2 flex justify-between">
                {isEditing ? (
                    <input
                        type="text"
                        name="channel_name"
